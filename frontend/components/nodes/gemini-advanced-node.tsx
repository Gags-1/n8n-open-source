...
interface GeminiAdvancedNodeData {
  apiKey?: string;
  model?: string;
  temperature?: number;
...
interface GeminiAdvancedNodeProps {
  data: GeminiAdvancedNodeData;
  id: string;
  selected?: boolean;
  onDelete?: (nodeId: string) => void;
}

export function GeminiAdvancedNode({
  data,
  id,
  selected,
  onDelete,
}: GeminiAdvancedNodeProps) {
  const [apiKey, setApiKey] = useState(data.apiKey || "");
  const [model, setModel] = useState(data.model || "gemini-1.5-flash");
  const [temperature, setTemperature] = useState(data.temperature || 0.7);
  const [maxOutputTokens, setMaxOutputTokens] = useState(
    data.maxOutputTokens || 2048
  );
  ...
  const [systemInstruction, setSystemInstruction] = useState(
    data.systemInstruction || data.prompt || ""
  );

  useEffect(() => {
    if (
      data.connectedInputs?.input &&
      data.connectedInputs?.input !== userPrompt
    ) {
      setUserPrompt(data.connectedInputs.input);
    }
    if (
      data.connectedInputs?.prompt &&
      data.connectedInputs?.prompt !== systemInstruction
    ) {
      setSystemInstruction(data.connectedInputs.prompt);
    }
    if (
      data.connectedInputs?.context &&
      data.connectedInputs?.context !== context
    ) {
      setContext(data.connectedInputs.context);
    }
  }, [data.connectedInputs, userPrompt, systemInstruction, context]);
  ...
  useEffect(() => {
    ...
    if (data.maxOutputTokens && data.maxOutputTokens !== maxOutputTokens)
      setMaxOutputTokens(data.maxOutputTokens);
    if (
      (data.userPrompt || data.input) &&
      (data.userPrompt || data.input) !== userPrompt
    )
      setUserPrompt(data.userPrompt || data.input || "");
    if (data.context && data.context !== context) setContext(data.context);
    if (
      (data.systemInstruction || data.prompt) &&
      (data.systemInstruction || data.prompt) !== systemInstruction
    )
      setSystemInstruction(data.systemInstruction || data.prompt || "");
  }, [
    data.apiKey,
    data.model,
    data.temperature,
  ...
  const isUserPromptConnected = data.connectedInputs?.input !== undefined;
  const isSystemInstructionConnected = data.connectedInputs?.prompt !== undefined;
  ...
  const displaySystemInstruction = isSystemInstructionConnected
    ? data.connectedInputs?.prompt || ""
    : systemInstruction;
  const displayContext = isContextConnected
    ? data.connectedInputs?.context || ""
    : context;
    ...
    >
      <CardHeader className="pb-3 px-4 pt-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center shadow-sm">
              <img
                src="/logo/gemini.svg"
                alt="Gemini"
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Multimodal AI with Advanced Features
              </p>
            </div>
          </div>
      ...
      <CardContent className="px-4 pb-4 space-y-3">
        <div className="space-y-1.5 relative">
          ...
          />
          <Label
            htmlFor={`gemini-adv-user-prompt-${id}`}
          ...
          <Textarea
            id={`gemini-adv-user-prompt-${id}`}
            placeholder={
              isUserPromptConnected
                ? "Value from connected node"
                : "Enter user prompt or connect from another node..."
            }
        ...
        <div className="space-y-1.5 relative">
          ...
          <Textarea
            id={`gemini-adv-context-${id}`}
            placeholder={
              isContextConnected
                ? "Value from connected node"
                : "Enter context information or connect from another node..."
            }
        ...
        <div className="space-y-1.5">
          <Label
            htmlFor={`gemini-adv-api-key-${id}`}
            className="text-xs font-medium text-slate-600 dark:text-slate-400"
          >
            API Key
          </Label>
          <Input
            id={`gemini-adv-api-key-${id}`}
            type="password"
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="space-y-1.5">
            <Label
              htmlFor={`gemini-adv-max-tokens-${id}`}
              className="text-xs font-medium text-slate-600 dark:text-slate-400"
            >
              Max Tokens
            </Label>
            <Input
              id={`gemini-adv-max-tokens-${id}`}
              type="number"
              min="1"
              max="8192"
              placeholder="2048"
